            ...
            <div class="card mg-b-20">
                <div class="pb-0 card-header">
                    <div class="d-flex justify-content-between">
                        <h4 class="card-title mg-b-0">عرض الموردين</h4>
                        <div class="card-toolbar">
                            <button class="btn btn-light">
                                <i class="mdi mdi-plus" style="color: black;"></i>
                                <a href="{{ url('inventory/supplier/create') }}" style="font: bolder; color:rgb(0, 0, 0);">
                                    اضافة الموردين </a>
                            </button>
                        </div>
                    </div>
                        ...
                        <table id="example">
                            ...
                            <tbody>
                                ...
                                @foreach ($supplier as $suppliers)
                                    ...
                                    <tr>
                                        <td> {{ $i }}</td>
                                        <td> {{ $suppliers->supplier_num }}</td>
                                        <td> {{ $suppliers->supplier_name }}</td>
                                        <td> {{ $suppliers->address }}</td>
                                        <td> {{ $suppliers->phone }}</td>
                                        <td> {{ $suppliers->ipn }}</td>
                                        <td> <img src="{{ asset('logo_supplier/' . $suppliers->logo) }}" alt="شعار المورد" height="50px" width="80px">
                                        </td>
                                        ...
                                        <td data-field="Actions" data-autohide-disabled="false" aria-label="null"
                                            ...
                                            {{-- <form method="POST"
                                                action="{{ url('/inventory/supplier' . '/' . $suppliers->id) }}"
                                            ...
                                            <a class="modal-effect btn btn-sm btn-danger btn-icon"
                                                data-effect="effect-scale" data-supplier_id="{{ $suppliers->id }}"
                                                data-supplier_name="{{ $suppliers->supplier_name }}" data-toggle="modal"
                                                href="#modaldemo8" title="حذف"><i class="la la-trash"></i></a>

                                            <a href="{{ url('/inventory/supplier/' . $suppliers->id . '/edit') }}"
                                                class="ml-2 mr-2 btn btn-sm btn-success btn-icon" title="Edit details"><i
                                                    class="la la-edit"></i></a>

                                            <a href="{{ url('/inventory/supplier/' . $suppliers->id) }}"
                                                title="View supplier"><button class="btn btn-sm btn-info btn-icon"> <svg
            ...
            <div class="modal-dialog modal-dialog-centered" role="document">
                <div class="modal-content modal-content-demo">
                    <div class="modal-header">
                        <h6 class="modal-title">حذف المورد</h6><button aria-label="Close" class="close"
